...
module.exports ={
    ...
    getTreeGridSQL:function(options){
        ...
        //{2}:TableName
        var sql= 
             ' select {4},sum(case when t3.'+'"Number"'+' is null then 0 else t3.'+'"Number"'+' end) as '+'"childrenCount"'
            +' from {2}  t1'
            +' left outer join (select {0},{1},1 as '+'"Number"'+' from {2})  t3 on t1.{0}=t3.{1} and t1.{1}<>t3.{1}';
        ...
        var sfields1='t1.*';
        //2018.03.16
        options.OIDField='t1.'+options.OIDField;
        options.keyFieldName='t1.'+options.keyFieldName;
        options.OIDParentField='t1.'+options.OIDParentField;
        if (options.fields){
            var fields1=[];
            for (var i=0;i<options.fields.length;i++){
                options.fields[i]='t1.'+options.fields[i];
                fields1[i]=options.fields[i] +' as '+'"'+options.fields[i].split(".")[1]+'"'; 
            }
            fields=options.fields.join();
            sfields1=fields1.join();
        }
        sql=yjUtils.formatStr(sql,options.OIDField.split(".")[1],
            options.OIDParentField.split(".")[1],options.tableName,fields,sfields1);
        return sql;
    },
    /**
     * 获取非树形sql查询语句。
    ...
    getDataGridSQL:function(options){       
        ...
        if (options.fields){
            for(var i=0;i<options.fields.length;i++){
                options.fields[i]=options.fields[i]+' as "'+options.fields[i]+'"'
            }
            fields=options.fields.join();
        }
    ...
    saveDelta:function(options){
        ...
        function checkResult(){
            if (finishedCount==totalCount){
                if (errors.length>0){
                    var err=new Error(JSON.stringify(errors));
        ...
        function saveDB(sql,params,method,result,index){
            ...
            yjDBService[method]({
                ...
                error : function(err){
                    ...
                    errors.push({errno:err.errno,code:err.code,message:err.message});
                    finishedCount++;
                    checkResult();
                }
            });
            ...
            for (var i=0;i<delta.updated.length;i++){
                ...
                for(var fieldName in row){
                    if (fieldName!=CNST_keyFieldName){
                        sql+=yjDBService.escapeId(fieldName)+"=:"+yjDBService.escapeId(fieldName)+",";
                        params.push(row[fieldName]);
                    }
                }
                sql=sql.substr(0,sql.length-1);

                sql+=" where "+CNST_keyFieldName+"=:"+CNST_keyFieldName;
        ...
        if (delta.deleted){
            for (var i=0;i<delta.deleted.length;i++){
                ...
                if (options.OIDField && options.OIDParentField){
                    var sql=" delete from {2} "+
                    "where OrgOID=(select t1.{0} from Orgs t1 left outer join Orgs t2 on "+
                    " t2.{1}=t1.{0} and t2.{1}<>t2.{0} where t1.{0}="+"'{3}' and t2.{0} is null）";  
    ...
    selectData:function(options){
        ...
        if(options.clob2String){
            deliverObj['clob2String']=options.clob2String;
        }
        yjDBService.selectData(deliverObj);
    },
    /**
