...
function getSQL_Where_TreeGrid(options){
	...
	if (OID!=null){
		...
		// 指定OID 此时只有一条记录page==null，查询是不会显示总记录数和页数，前端显示会有错 eg.showOrgs页面
		options.req.query.page=1
...
module.exports ={
	...
	selectData:function(options){
		...
		yjDBService.selectData({
			...
			pageIndex: pageIndex,
			pageRowCount : options.req.query.rows?parseInt(options.req.query.rows):options.pageRowCount,
			orderBy: sql_OrderBy,
